	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			for (int i=0; i <= trials; i++) {
				Document basic = new BasicDocument(doc);
				double flsb = basic.getFleschScore();
			}
			long endTimeBasic = System.nanoTime();
			double durationBasic = (endTimeBasic - startTimeBasic) /  1000000000.0;
			System.out.print(durationBasic + "\t");
			
			long startTimeEff = System.nanoTime();
			for (int i=0; i <= trials; i++) {
				Document eff = new EfficientDocument(doc);
				double flse = eff.getFleschScore();
			}
			long endTimeEff = System.nanoTime();
			double durationEff = (endTimeEff - startTimeEff) /  1000000000.0;
			System.out.print(durationEff + "\n");
