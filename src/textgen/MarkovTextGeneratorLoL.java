	...
	public void train(String sourceText)
	{
		...
		for (int i = 1; i < words.length; i++) {
			...
			for (ListNode word : wordList) {
				if (word.getWord().equals(prevWord)) {
					word.addNextWord(words[i]);
					found = true;
				}
			}
			if (!found) {
				ListNode curr = new ListNode(prevWord);
				curr.addNextWord(words[i]);
				wordList.add(curr);
			}
			prevWord = words[i];
		}
		...
		while (count < numWords) {
			for (ListNode word : wordList) {
				if (word.getWord().equals(currWord)) {
					String randWord = word.getRandomNextWord(rnGenerator);
